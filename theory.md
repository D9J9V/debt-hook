            ...
            - **Inmediatamente después del préstamo (t=0):**
                - **Activos:** `1 ETH (colateral) + 1000 USDC (efectivo recibido) = 2000 + 1000 = 3000 USDC`
                    - **Pasivos:** `1000 USDC (deuda)`
                        ...
                        D1​=D0​⋅ert=1000⋅e0.1⋅1≈1000⋅1.10517=1105.17 USDC
                        ...
                        **Escenario B: El precio de ETH disminuye a 1000 USDC/ETH**
                        ...
                        - **Valor de la Deuda (Dt​):** Es una función determinista que crece exponencialmente con el tiempo. El principal es `$D_0$`. Dt​=D0​⋅ert
                        - **Valor del Colateral (Ct​):** Es un proceso estocástico, ya que depende del precio de mercado del activo, `$P_t$`. Ct​=Q⋅Pt​ Donde `$Q$` es la cantidad de colateral (ej. 1 ETH) y `$P_t$` es el precio (ej. `USDC/ETH`).
                                                ...
                                                > - **Eje X:** Tiempo (desde `t=0` hasta `T=1`)
                                                    > - **Curva de la Deuda:** Una función exponencial creciente, que parte de `$D_0$` y termina en `$D_0 \cdot e^{rT}$`.
                                                        > - **Curva del Colateral:** Un camino aleatorio (proceso estocástico) que parte de `$C_0$`. La liquidación se activa si `$C_t$` intersecta a `$D_t$`.
                                                            ...
                                                            ### Funciones de Pago y Similitudes con Derivados

                                                            Vamos a examinar las funciones de pago para cada participante.

                                                            La introducción de una cláusula de liquidación anticipada es lo que transforma este producto financiero de una estructura simple a una más compleja y realista, similar a los derivados exóticos.
